        ...
        <div className='grid grid-cols-4 gap-2 p-4'>
          {colors.map((color) => (
            <button
              key={color.value}
              className={cn(
                'flex flex-col items-center gap-1 rounded-md p-2 text-xs transition-colors hover:bg-gray-100',
                value === color.value && 'bg-gray-100 ring-2 ring-blue-500'
              )}
              onClick={() => onChange(color.value)}
            >
              <div className='h-8 w-8 rounded-full border border-gray-300' style={{ backgroundColor: color.color }} />
